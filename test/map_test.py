...
d = pbt.timing.dirfile_interp('idx_roach', path_master=master_file, path_roach=roach_file_path, \
                              idx_start=idx_start, idx_end=idx_end, roach_num=str(int(roach_number)), \
                              offset=time_offset)
...
t = Time(d.time_master, format='unix', scale ='utc')
loc = EarthLocation(pointing_data_resample['LON']*u.deg, pointing_data_resample['LAT']*u.deg, height=pointing_data_resample['ALT']*u.m)
...
#final_ra = radec[0].copy()
#final_dec = radec[1].copy()


#wcs.wcs_proj(final_ra, final_dec, np.shape(phase)[0])
#w = wcs.pixel.copy()

#maps=pbt.mapmaker.mapmaking(phase, 1., 0., w, crpix)

#maps = maps.binning_map(coadd=False)

#pbt.plot.plot_map(maps['I']['det_0'], wcs.w.deepcopy(), w)
